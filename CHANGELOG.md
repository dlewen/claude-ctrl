...
### Fixed
- `fix/ls-glob`: Replace `ls .active-*` glob with `find -name '.active-*'` in check-guardian.sh post-guardian health check — bare `ls` crashes under `pipefail` when no `.active-*` files exist, killing the entire hook
- `fix/guardian-dedup2`: Add early-exit dedup path to check-guardian.sh — when the dedup marker exists and findings are unchanged, exit before any tracking/logging side effects; eliminates all stderr output that Claude Code was capturing as "Stop hook feedback" and breaking the SubagentStop loop (DEC-GUARDIAN-DEDUP-001 early-exit extension)
- `fix/guardian-dedup`: Dedup SubagentStop output in check-guardian.sh — hash findings and compare to marker file so repeated end_turn cycles with identical findings go silent instead of re-delivering context; task-track.sh clears the dedup marker on new guardian dispatch so the first SubagentStop always delivers (DEC-GUARDIAN-DEDUP-001)
- `fix/guardian-loop`: Break SubagentStop feedback loop in check-guardian.sh by replacing additionalContext JSON output with `echo '{}'`; migrate guardian Phase B proof transition (verified->committed) to post-task.sh where it fires reliably via PostToolUse:Task (DEC-POST-TASK-GUARDIAN-001)
- `fix/proof-selfheal`: Fix `write_proof_status()` in log.sh silently failing (exit 1) when `proof_state_set` was not pre-loaded — now auto-loads state-lib.sh via `require_state` on demand; agents that source source-lib.sh without calling require_state no longer lose proof-status writes (DEC-LOG-SELFHEAL-001)
- `fix/grep-c-fallback`: Replace `grep -c ... || echo "0"` anti-pattern with `|| true` across 12 hook files — the old pattern produced `0\n0` (two lines) when grep matched nothing, causing `[[ "$VAR" -eq 0 ]]` integer comparison to crash with a syntax error
